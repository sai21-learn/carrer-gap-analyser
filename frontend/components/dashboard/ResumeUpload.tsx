...
export default function ResumeUpload() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setFile(null);
      // In a real app, we might trigger a refresh of the dashboard data here
    } catch (err) {
      setUploadStatus('error');
    ...
    <div className="rounded-xl border bg-white p-6 shadow-sm dark:bg-zinc-950">
      <div className="mb-4">
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          Upload your latest resume (PDF) to analyze your skills and get better recommendations.
        </p>
      </div>
      ...
      <div
        ...
        className={cn(
          ...
          file ? "border-blue-500 bg-blue-50/50 dark:bg-blue-900/10" : "border-zinc-200 hover:border-zinc-300 dark:border-zinc-800 dark:hover:border-zinc-700"
        )}
      >
        <input
          type="file"
      ...
      {uploadStatus === 'success' && (
        <div className="mt-4 flex items-center space-x-2 rounded-lg bg-emerald-50 p-3 text-sm text-emerald-700 dark:bg-emerald-900/20 dark:text-emerald-400">
          ...
          <span>Resume successfully analyzed! Your skills are being updated.</span>
        </div>
      )}
